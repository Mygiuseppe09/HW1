...
function checkIfThePostIsLikedByLoggedUser(postId) {
    fetch("Json/bool_like.php" + "?id=" + postId).then(onResponse).then(setLike);
}
...
function refreshAllPostsLikes() {
    ...
    for (let post of posts) {
        ...
        fetch("Json/get_post_likes.php" + '?id=' + filtred_postId).then(onResponse).then(onNlikesJson);
    }
}
...
function openLikeModalView(event) {
    ...
    fetch("Json/get_likes_of_a_post.php" + "?id=" + postId).then(onResponse).then(onLikesJson);
}
...
function setLike(json) {
    ...
    if (json.is_liked === 'TRUE')
        ...
        likeButton.src = "assets/icons/liked.png";
    else
        likeButton.src = "assets/icons/like_empty.png";
}
...
function putLikeOnPost(event) {
    ...
    fetch("Json/bool_like.php" + "?id=" + postId).then(onResponse).then(onBoolLikeJson);
}
...
function getUserInfo(event) {
    ...
    fetch('Json/get_user_info.php' + '?username=' + nomeUtente).then(onResponse).then(onUserByUsernameJson);
...
function searchUsersByPlace(event) {
    ...
    if (input) {
        ...
        fetch('Json/get_users_by_place.php' + '?city=' + encodedInput).then(onResponse).then(onUsersByPlaceJson);
    }
}
...
function onLikesJson(json) {
    ...
    for (let user of json) {
        /*
            <div class="user">
                        <div class="like_avatar_container image_container"><img class="image" src="assets/icons/girl_avatar.png" alt=""></div>
                        <h4>@Maria_Rossi</h4>
                    </div>
        */
        ...
        if (user.sesso === 'M')
            avatarIMG.src = 'assets/icons/avatar.png';
        else avatarIMG.src = 'assets/icons/girl_avatar.png';
...
function finallyPutLike(json) {
    ...
    likeIMG.src = "assets/icons/liked.png"
...
function finallyRemoveLike(json) {
    ...
    // DEcoloriamo il like
    const likeIMG = document.querySelector('#postID'+ postId + ' img.like_button');
    likeIMG.src = "assets/icons/like_empty.png"
...
function onBoolLikeJson(json) {
    ...
    if (json.is_liked === 'FALSE') {
        ...
        fetch("Json/store_new_like.php" + "?id=" + postId).then(onResponse).then(finallyPutLike);

    }
    else {
        // va tolto il mi piace
        fetch("Json/delete_like.php" + "?id=" + postId).then(onResponse).then(finallyRemoveLike);
        ...
        for (let post of json) {
            ...
            iconImg.classList.add('image');
                if (post.user_gender === 'M')
                    iconImg.src = 'assets/icons/avatar.png';
                else
                    iconImg.src = 'assets/icons/girl_avatar.png';
    ...
    setTimeout(() => {
        ...
        if (json.length === 0) {
            ...
            for (let user of json) {
                ...
                const iconIMG = document.createElement('img');
                iconIMG.classList.add('image');
                iconIMG.src = 'assets/icons/generic_user.png';
...
function onUserJson(json) {
        ...
        let src = undefined;
        if (json.Sesso === 'F')
            src = 'assets/icons/girl_avatar.png';
        else
        ...
        // inizializzazione
        name.innerText = json.Nome + ' ' + json.Cognome;
        username.innerText = '@' + json.Username;
        icon.src = src;
        nPosts.innerText = json.nposts;
        nPlaces.innerText = json.numero_luoghi_visitati;
...
function onPlacesJson(json) {
        ...
        for (let place of json) {
            ...
            pinIcon.src = 'assets/icons/pin.png';
...
fetch("Json/get_places_visited_by_logged_user.php").then(onResponse).then(onPlacesJson);
fetch("Json/get_logged_user_info.php").then(onResponse).then(onUserJson);
fetch('Json/get_feed_section.php').then(onResponse).then(onPostsJson);
