...
function deletePost(event) {
    ...
    fetch("Json/delete_post.php" + "?id=" + postId).then(onResponse).then(onDeletePostJson);
}
...
function onDeletePostJson(json) {
    ...
    if (json.deleted === 'TRUE') {
        ...
        setTimeout(() => {
            fetch('Json/get_logged_user_posts.php').then(onResponse).then(onPostsJson);
...
function onPostsJson(json) {
    if (json.length === 0) {
        ...
        for (let post of json) {
            ...
            textPost.innerText = '"' + post.name_place + '"';
